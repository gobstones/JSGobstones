...
class CmdApplication(object):
    ...
    def start(self):
        self.initialize()
        arguments, options = get_options(self.get_option_switches())
        
        option_processed = None
        
        for option_switch, option_handler in self.get_option_handlers().iteritems():
            option = option_switch.split(" ")[0]
            if options[option]:
                if isinstance(options[option], list):
                    option_handler(*options[option])
                else:
                    option_handler()
                option_processed = option
                break
            
        if option_processed is None:
